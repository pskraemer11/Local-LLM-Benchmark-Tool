...
- [x] **18. qwen3.5-9b: Reasoning-Einstufung vs. Hub klären** — **ABGESCHLOSSEN (06.08.)** — Registry `qwen/qwen3.5-9b` auf `reasoning: thinking` angeglichen + `max_context_length: 262144` nachgetragen (Hub-Wert); Kommentar `assemble_blueprint.py:248-249` korrigiert (Dual-Mode, Default Thinking — `enableThinking.defaultValue: true` laut Hub model.yaml/Qwen-Model-Card); enable_thinking-Handling geprüft: `get_model_config` erzwingt `enable_thinking: True` im Thinking-Lauf (REASONING_PATTERNS enthält qwen3.5/qwen3.6), `run_benchmarks` hatte `_is_qwen3_6_model()` bereits, `custom_benchmark._model_supports_reasoning()` liest jetzt korrekt `thinking` — kein weiterer Code-Change nötig. Verifiziert: `validate --repro` = **0 Hub-Abweichungen**, ruff 0, 741 Tests grün. Hintergrund bleibt dokumentiert:
  ...
  - Alle vier Teilaufgaben (a)–(d) abgearbeitet: (a) Registry auf `thinking` angeglichen, (b) `max_context_length: 262144` nachgetragen, (c) Kommentar korrigiert, (d) Handling geprüft (kein Code-Change nötig).
