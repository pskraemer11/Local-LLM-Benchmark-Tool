...
|------|------:|------|------|
| `run_benchmarks.py` | 1.287 | Launcher, 4 Pipelines | stabil, reif |
| `custom_benchmark.py` | 1.920 | DS1000/CoderEval, Monitor | stabil, reif |
| `consolidate_results.py` | 1.488 | Ranking-Aggregation | stabil, reif |
...
- `consolidate_results.py --compare` mit Paired Bootstrap CI
...
**Status:** ✅ Behoben – `get_available_models()` filtert jetzt auf `m["key"] + " " + m["display"]` (Konkatenation), und redundante Filterung in `run_benchmarks.py` und `custom_benchmark.py` entfernt.

### 2.2 `LB_MEANS_BLACKLIST` Case-Sensitivity

`benchmark_config.py:284 = {"Granite 4.0 H Tiny"}` ist als String definiert, `consolidate_results.py` filtert vermutlich auf `modelKey == "granite-4.0-h-tiny"`. Case-Sensitivity unklar.
...
**Status:** ✅ Behoben – `MMLU_PRO_ENABLED` entfernt. `MMLU_PRO_SUBSETS` bleibt (wird in `consolidate_results.py:786` defensiv genutzt).
...
**Status:** ✅ Behoben – Kommentar ergänzt, der die Intention (Server-Antwort noch nicht bereit, retry im nächsten Loop) dokumentiert. `_safe_float()` Helfer in `custom_benchmark.py` führt 4-fach `try/except: pass` zu einer sauberen Funktion zusammen.

### 3.3 Magic Strings

`model_manager.py:428` nutzt `"check"` als Sentinel-Modellname, `run_benchmarks.py:682` nutzt `"local-model"` für evalplus.
...
Pre-existing: `cannot import name 'SAFE_CONTEXT' from 'run_benchmarks'`. Auch 11 Tests testeten veraltete `_get_lmeval_params`-Kaskade (vor `Variante C+`).
...
**Status:** ⚪ Dokumentiert – Code-Kommentar in `run_benchmarks.py:431-436` ergänzt, der die aktuelle Single-Thread-Annahme festhält und bei zukünftigem Parallel-Benchmarking `threading.Lock` empfiehlt.
...
**Status:** ✅ Behoben – 3+ Zeilen Code in `custom_benchmark.py:1154-1169` ergänzt, die synthetisches `def expected_func(*args, **kwargs): <body>` generiert. Test-Erwartung korrigiert: prüft jetzt `return x * 2` im Wrapper statt `pass`.
...
| `consolidate_results.py` | `MMLU_PRO_ENABLED` Import entfernt |
| `custom_benchmark.py` | Monitor-Sampling 0.5s, `_safe_float()` Helfer, `_validate_model_key`-Test, `_unwrap_solution_for_insert` Bug-Fix |
...
| `run_benchmarks.py` | Redundante EXCLUDE-Filterung entfernt, `EVALPLUS_SENTINEL_MODEL` Konstante, `THINKING_ENABLED` dokumentiert |
