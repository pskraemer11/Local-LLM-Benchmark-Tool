"""
Zentrale Konfiguration fuer alle Benchmark-Pipelines.

Wird importiert von:
  - run_benchmarks_v11.py       (PIPELINE_DISCOVERY, TOOL_EVAL_SCENARIO_IDS)
  - consolidate_results_v11.py  (DISPLAY_NAMES, WHITELIST, QUANT_MAP, CAT_WEIGHTS, ...)
  - generate_quant_map.py       (DISPLAY_NAMES, WHITELIST)
  - tests/test_scores.py        (CAT_WEIGHTS, OVERALL_WEIGHTS)

Quellen-Prioritaet fuer QUANT_MAP:
  1. QUANT_MAP (statisch, hier unten) – zuverlaessig, auch fuer geloeschte Modelle
  2. lms ls --json (dynamisch) – nur installierte Modelle
  3. LM Studio Config-Dateien
  4. GGUF-Metadaten-Cache

Bei neuen Modellen: python generate_quant_map.py --write ausfuehren.
"""

DISPLAY_NAMES = {
    "granite-4.1-8b":                        "Granite 4.1 8B",
    "mathstral-7b-v0.1":                     "Mathstral 7B v0.1",
    "microsoft/phi-4":                       "Phi 4",
    "unsloth/phi-4":                        "Phi 4 (unsloth)",
    "essentialai/rnj-1":                     "Rnj 1",
    "mistralai/mistral-nemo-instruct-2407":  "Mistral Nemo Instruct 2407",
    "mistralai/codestral-22b-v0.1":          "Codestral 22B v0.1",
    "qwen/qwen3.5-9b":                      "Qwen3.5 9B",
    "starcoder2-15b-instruct-v0.1":          "Starcoder2 15B Instruct v0.1",
    "pandalyst_13b_v1.0":                    "Pandalyst 13B v1.0",
    "wizardcoder-python-13b-v1.0":          "Wizardcoder Python 13B v1.0 I1",
    "januscoder-14b":                       "JanusCoder 14B UD",
    "ernie-4.5-21b-a3b-pt":                 "ERNIE 4.5 21B A3B PT",
    "qwen2.5-math-7b-instruct":              "Qwen2.5 Math 7B Instruct",
    "falcon3-10b-instruct":                  "Falcon3 10B Instruct",
    "devstral-small-2-24b-instruct-2512":    "Devstral Small 2 24B Instruct 2512 UD",
    "deepseek-coder-33b-instruct":           "Deepseek Coder 33B Instruct",
    "deepseek-coder-6.7b-instruct":          "Deepseek Coder 6.7B Instruct",
    "granite-4.0-h-tiny":                    "Granite 4.0 H Tiny",
    "ministral-3-14b-instruct-2512":         "Ministral 3 14B Instruct 2512",
    "qwen3-14b":                            "Qwen3 14B UD",
    "qwen3-coder-reap-25b-a3b-i1@iq4_xs":  "Qwen3 Coder REAP 25B A3B I1 @ IQ4_XS",
    "qwen3-coder-reap-25b-a3b-i1@q3_k_m":  "Qwen3 Coder REAP 25B A3B I1 @ Q3_K_M",
    "qwen3.5-9b-deepseek-v4-flash":          "Qwen3.5 9B DeepSeek v4 Flash",
    "qwen2.5-coder-14b-instruct":            "Qwen2.5 Coder 14B Instruct",
    "gemma-4-19b-a4b-it-reap-i1":           "Gemma 4 19B A4B Instruct REAP I1",
    "mellum2-12b-a2.5b-instruct":          "Mellum2 12B A2.5B Instruct",
    "mellum2-12b-a2.5b-instruct_moe":      "Mellum2 12B A2.5B Instruct MXFP4 MoE",
    "nerdsking-python-coder-7b-i":         "Nerdsking Python Coder 7B I Q8 0 I",
    "codellama-13b-python":                "Codellama 13B Python",
    "qwen2.5-coder-32b-instruct":          "Qwen2.5 Coder 32B Instruct",
    "qwen2.5-14b-instruct-1m":             "Qwen2.5 14B Instruct 1M",
    "qwen3.6-27b":                         "Qwen3.6 27B",
    "qwen3-30b-a3b-python-coder":          "Qwen3 30B A3B Python Coder",
    "gpt-oss-20b":                        "GPT-OSS 20B",
    "gemma-4-26b-a4b-it":                "Gemma 4 26B A4B Instruct UD",
    "qwen3.6-28b-reap-i1":               "Qwen3.6 28B REAP I1",
    "acemath-7b-instruct":               "AceMath 7B Instruct",
    "lmstudio-community/gpt-oss-20b":    "GPT-OSS 20B (lmstudio)",
    "unsloth/gpt-oss-20b":               "GPT-OSS 20B (unsloth)",
}

# Nur Modelle aus dem aktuellen Dense-Run (v10 + Step1 + Step2, Juli 2026)
# SampleSize=20, Bugfixes aktiv (Regex, csv_writer timestamp)
WHITELIST = [
    "qwen3.6-28b-reap-i1",
    "qwen3-coder-reap-25b-a3b-i1@iq4_xs",
    "qwen3-coder-reap-25b-a3b-i1@q3_k_m",
    "lmstudio-community/gpt-oss-20b",
    "unsloth/gpt-oss-20b",
    "qwen2.5-14b-instruct-1m",
    "qwen3-30b-a3b-python-coder",
    "acemath-7b-instruct",
    "qwen3.6-27b",
    "gemma-4-19b-a4b-it-reap-i1",
    "mellum2-12b-a2.5b-instruct",
    "nerdsking-python-coder-7b-i",
    "mellum2-12b-a2.5b-instruct_moe",
    "granite-4.0-h-tiny",
    "qwen2.5-coder-14b-instruct",
    "granite-4.1-8b",
    "unsloth/phi-4",
    "devstral-small-2-24b-instruct-2512",
    "falcon3-10b-instruct",
    "ernie-4.5-21b-a3b-pt",
    "essentialai/rnj-1",
    "mistralai/codestral-22b-v0.1",
]

# Auto-generated by generate_quant_map.py – do not edit manually.
# Run 'python generate_quant_map.py --write' to update.
# Source: lms ls --json + LM Studio configs + GGUF cache
# Conflicts resolved: Steckbrief > Config > GGUF-Cache > Filename
QUANT_MAP = {
    "codellama-13b-python": "Q6_K",  # Codellama 13B Python
    "mistralai/codestral-22b-v0.1": "IQ4_XS",  # Codestral 22B v0.1
    "deepseek-coder-33b-instruct": "Q3_K_S",  # Deepseek Coder 33B Instruct
    "deepseek-coder-6.7b-instruct": "Q8_0",  # Deepseek Coder 6.7B Instruct
    "devstral-small-2-24b-instruct-2512": "IQ3_XXS",  # Devstral Small 2 24B Instruct 2512 UD
    "ernie-4.5-21b-a3b-pt": "IQ4_NL",  # ERNIE 4.5 21B A3B PT
    "falcon3-10b-instruct": "Q8_0",  # Falcon3 10B Instruct
    "gpt-oss-20b": "Q6_K",  # GPT-OSS 20B
    "gemma-4-19b-a4b-it-reap-i1": "IQ4_NL",  # Gemma 4 19B A4B Instruct REAP I1
    "granite-4.0-h-tiny": "Q8_0",  # Granite 4.0 H Tiny
    "granite-4.1-8b": "Q8_0",  # Granite 4.1 8B
    "januscoder-14b": "Q6_K",  # JanusCoder 14B UD
    "mathstral-7b-v0.1": "Q8_0",  # Mathstral 7B v0.1
    "mellum2-12b-a2.5b-instruct": "Q4_K_M",  # Mellum2 12B A2.5B Instruct
    "mellum2-12b-a2.5b-instruct_moe": "MXFP4",  # Mellum2 12B A2.5B Instruct MXFP4 MoE
    "ministral-3-14b-instruct-2512": "Q6_K",  # Ministral 3 14B Instruct 2512
    "nerdsking-python-coder-7b-i": "Q8_0",  # Nerdsking Python Coder 7B I Q8 0 I
    "pandalyst_13b_v1.0": "Q6_K",  # Pandalyst 13B v1.0
    "microsoft/phi-4": "Q5_K_M",  # Phi 4
    "unsloth/phi-4": "Q5_K_M",  # Phi 4 (unsloth)
    "qwen2.5-14b-instruct-1m": "Q6_K",  # Qwen2.5 14B Instruct 1M
    "qwen2.5-coder-14b-instruct": "Q5_0",  # Qwen2.5 Coder 14B Instruct
    "qwen2.5-coder-32b-instruct": "Q3_K_S",  # Qwen2.5 Coder 32B Instruct
    "qwen2.5-math-7b-instruct": "Q8_0",  # Qwen2.5 Math 7B Instruct
    "qwen3-30b-a3b-python-coder": "Q3_K_S",  # Qwen3 30B A3B Python Coder
    "qwen3-coder-reap-25b-a3b-i1": "Q3_K_M",  # Qwen3 Coder REAP 25B A3B I1
    "qwen/qwen3.5-9b": "Q5_K_M",  # Qwen3.5 9B
    "qwen3.5-9b-deepseek-v4-flash": "Q8_0",  # Qwen3.5 9B DeepSeek v4 Flash
    "qwen3.6-27b": "Q3_K_S",  # Qwen3.6 27B
    "essentialai/rnj-1": "Q8_0",  # Rnj 1
    "starcoder2-15b-instruct-v0.1": "Q6_K",  # Starcoder2 15B Instruct v0.1
    "wizardcoder-python-13b-v1.0": "Q6_K",  # Wizardcoder Python 13B v1.0 I1
    "gemma-4-26b-a4b-it": "IQ3_S",  # Gemma 4 26B A4B Instruct UD
    "qwen3.6-28b-reap-i1": "IQ3_M",  # Qwen3.6 28B REAP I1
    "acemath-7b-instruct": "Q8_0",  # AceMath 7B Instruct
    "lmstudio-community/gpt-oss-20b": "MXFP4",  # GPT-OSS 20B (lmstudio)
    "unsloth/gpt-oss-20b": "Q6_K",  # GPT-OSS 20B (unsloth)
}


MMLU_PRO_SUBSETS = [
    "mmlu_pro_biology", "mmlu_pro_business", "mmlu_pro_chemistry",
    "mmlu_pro_computer_science", "mmlu_pro_economics", "mmlu_pro_engineering",
    "mmlu_pro_health", "mmlu_pro_history", "mmlu_pro_law",
    "mmlu_pro_math", "mmlu_pro_other", "mmlu_pro_philosophy",
    "mmlu_pro_physics", "mmlu_pro_psychology",
]

LB_MEANS_BLACKLIST = {"Granite 4.0 H Tiny"}

CAT_WEIGHTS = {
    "coding": {
        "HumanEval+_plus": 0.25,
        "MBPP+_plus": 0.25,
        "DS1000": 0.25,
        "CoderEval": 0.25,
    },
    "knowledge": {
        "ARC-Challenge": 0.25,
        "HellaSwag": 0.25,
        "TruthfulQA": 0.25,
        "MMLU-Pro": 0.25,
    },
    "math": {
        "MathQA": 1.0,
    },
    "agentic": {
        "Agentic": 1.0,
    },
}

OVERALL_WEIGHTS = {"coding": 0.35, "math": 0.25, "agentic": 0.25, "knowledge": 0.15}

PIPELINE_TIMEOUTS = {
    "custom_subprocess": 14400,
    "evalplus_base": 600,
    "lmeval_base": 600,
    "mmlupro_per_subset": 300,
    "agentic_subprocess": 3600,
    "agentic_scenario": 600,
}

TOOL_EVAL_SCENARIO_IDS = [f"TC-{i:02d}" for i in range(1, 70)]

PIPELINE_DISCOVERY = {
    "custom": {"module": None, "glob": "custom_benchmark_v*.py", "desc": "Subprozess (DS1000, CoderEval)"},
    "evalplus": {"module": "evalplus", "glob": None, "desc": "Python-Modul (HumanEval+, MBPP+)"},
    "lmeval": {"module": "lm_eval", "glob": None, "desc": "Python-Modul (ARC, HellaSwag, ...)"},
    "agentic": {"module": "tool_eval_bench", "glob": None, "desc": "Python-Modul (Tool-Evaluation)"},
}
